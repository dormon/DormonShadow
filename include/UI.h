...
namespace ui{
  ...
  inline std::vector<Element*>repear1D(size_t N,std::function<Element*(size_t)>const&fce){
    std::vector<Element*>result;
    for(size_t i=0;i<N;++i)
      result.push_back(fce(i));
    return result;
  }
  inline std::vector<std::vector<Element*>>repear2D(size_t X,size_t Y,std::function<Element*(size_t,size_t)>const&fce){
